...
@IBDesignable public class HBSegmentedControl: UIControl {
    ...
    @IBInspectable public var unselectedLabelColor: UIColor = UIColor.gray {
        didSet { setSelectedColors() }
    }
    ...
    private func setupLabels() {
        ...
        for index in 1...items.count {
            ...
            //MARK: - Перенести в отдельный метод
            let lineView = UIView()
            label.addSubview(lineView)
            lineView.translatesAutoresizingMaskIntoConstraints = false
            lineView.leadingAnchor.constraint(equalTo: label.leadingAnchor, constant: 11).isActive = true
